...
{if $mgrpname|in_group || $admgrpname|in_group}
	...
	<h3>{$prefs.ta_syn_organicgrp_sterm} Name and Description</h3>
	...
	{wikiplugin _name="tracker" trackerId="{addonobjectid profile="004_og_tracker" ref="trk_og"}" fields="{addonobjectid profile="004_og_tracker" ref="trk_og_title"}:{addonobjectid profile="004_og_tracker" ref="trk_og_description"}:{addonobjectid profile="004_og_tracker" ref="trk_og_logo_image"}" action="Change" url="syn_organicgrp_managegrp?itemId={$f_itemId}"}{/wikiplugin}
	<h3>Removing {$prefs.ta_syn_organicgrp_sterm}</h3>
	<p>Script to remove {$prefs.ta_syn_organicgrp_sterm} is under development. For now, please contact us if you wish to remove the {$prefs.ta_syn_organicgrp_sterm}.<p></p>
{else}
